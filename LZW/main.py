class LZW:
    ...
    def compressed(self,data):
        list_of_code = {chr(i): i for i in range(128)}
        data_comperssed = []
        i=0
    ...
    def decompressed(self,compressed_data):
        list_of_code={i: chr(i) for i in range(128)}
        data=chr(compressed_data[0])
        i=1
        key_of_list_code=128
        while i<len(compressed_data):
            index=len(data)
            start= index-len(list_of_code[compressed_data[i-1]])
            if compressed_data[i] not in list_of_code.keys():
